...
<html lang="en" dir="ltr">
  ...
  <body>
    <div id=center class="container-fluid">
      <div class="col- col-sm col-md col-lg col-xl containera animated fadeInUp delay-02s" style="background: url(../mv-includes/images/cine-bg.jpg) left top repeat !important;">
            ...
            <!-- Validation and Login to the respective Home Pages for all users -->
                        ...
                        $errors = array();
                          if(isset($_POST['login'])){
                            ...
                            if(empty($user_pasd)) { array_push($errors,"Password required");}
                            if(count($errors) == 0){ #if no errors search and find username from the table
                              $psd = md5($user_pasd);
                              $logquery = "SELECT user_name,user_type FROM tbl_user WHERE user_uname='$user_uname' AND user_pasd='$psd'";
                              ...
                              if(mysqli_num_rows($results) > 0){ #if there is a result check if the usertype is admin or user
                                ...
                                $tlogquery = "SELECT thr_name,thr_status FROM tbl_theater WHERE thr_uname='$user_uname' AND thr_pasd='$psd'";
